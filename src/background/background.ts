...
async function performHealthCheck(): Promise<{
  ...
  try {
    ...
    // 检查配置管理器
    const configHealth = await configManager
      .getConfig()
      .then(() => ({ healthy: true, issues: [] }))
      .catch(error => ({
        healthy: false,
    ...
    return {
      allHealthy: false,
      results: [
        { component: 'HealthCheck', healthy: false, issues: [error.message] },
      ],
    }
  }
}
  ...
  try {
    ...
    switch (message.type) {
      case 'PROCESS_NOTE_DATA':
        handleProcessNoteData(message.data, sender)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
      ...
      case 'BATCH_PROCESS_DATA':
        handleBatchProcessData(message.dataList)
          .then(results => {
      ...
      case 'UPDATE_FEISHU_CONFIG':
        handleUpdateFeishuConfig(message.config)
          .then(() => {
      ...
      case 'GET_FEISHU_FIELDS':
        handleGetFeishuFields(message.tableId)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
      ...
      case 'UPDATE_FIELD_MAPPING':
        handleUpdateFieldMapping(message.mapping)
          .then(() => {
            sendResponse({ success: true })
          })
          .catch(error => {
      ...
      case 'CREATE_CONFIG':
        handleCreateConfig(message.config)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
      ...
      case 'UPDATE_CONFIG':
        handleUpdateConfig(message.id, message.updates)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
      ...
      case 'SET_ACTIVE_CONFIG':
        handleSetActiveConfig(message.id)
          .then(() => {
            sendResponse({ success: true })
          })
          .catch(error => {
      ...
      case 'EXPORT_CONFIG':
        handleExportConfig(message.password)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
      ...
      case 'IMPORT_CONFIG':
        handleImportConfig(message.configJson, message.password)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
      ...
      case 'DETECT_FIELD_MAPPING':
        handleDetectFieldMapping(message.tableId)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
      ...
      case 'PROCESS_FILES':
        handleProcessFiles(message.urls)
          .then(result => {
            sendResponse(result)
          })
          .catch(error => {
...
// 处理笔记数据
async function handleProcessNoteData(
  data: any,
...
async function handleDetectFieldMapping(tableId: string) {
  try {
    ...
    const suggestions = await connectionTester.detectFieldMapping(
      tableId,
