...
export class ConnectionTester {
  ...
  async testConnection(config: FeishuConfig): Promise<ConnectionTestResult> {
    ...
    try {
      ...
      if (!tokenValidation.success) {
        errors.push(
          `Access Token格式错误: ${tokenValidation.errors.join(', ')}`
        )
  ...
  // 测试权限
  private async testPermissions(): Promise<{
    success: boolean
    error?: string
  }> {
    try {
      ...
      const response = await fetch(
        `${this.client.getConfig().baseUrl}/open-apis/authen/v1/user_info`,
        {
          method: 'GET',
          headers: {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '权限测试失败',
      }
    }
  }

  // 测试表格访问
  private async testTableAccess(): Promise<{
    success: boolean
    error?: string
  }> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '表格访问失败',
      }
    }
  }

  // 测试数据操作
  private async testDataOperations(): Promise<{
    success: boolean
    error?: string
  }> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : '数据操作测试失败',
      }
    }
  }
  ...
  async validateAccessToken(
    token: string
  ): Promise<{ valid: boolean; errors: string[]; expiresAt?: number }> {
    const errors: string[] = []
    ...
    try {
      ...
      errors.push(
        `Token验证失败: ${error instanceof Error ? error.message : '未知错误'}`
      )
  ...
  // 验证表格ID
  async validateTableId(
    tableId: string,
    baseUrl: string = 'https://open.feishu.cn'
  ): Promise<{ valid: boolean; errors: string[]; tableInfo?: any }> {
    const errors: string[] = []
    ...
    try {
      ...
      errors.push(
        `表格ID验证失败: ${error instanceof Error ? error.message : '未知错误'}`
      )
      return { valid: false, errors }
    }
  }
  ...
  async detectFieldMapping(
    ...
    try {
      ...
      const mappingRules = [
        {
          source: 'title',
          patterns: ['标题', 'title', '名称', 'name', '主题'],
        },
        {
          source: 'author',
          patterns: ['作者', 'author', '用户', 'user', '发布者'],
        },
        {
          source: 'content',
          patterns: ['正文', 'content', '内容', '描述', 'description'],
        },
        ...
        {
          source: 'createTime',
          patterns: ['创建时间', 'create', 'time', '日期', 'date'],
        },
      ]
  ...
  private calculateConfidence(
    ...
    if (
      commonMappings[sourceField as keyof typeof commonMappings]?.includes(
        targetField
      )
    ) {
      return 0.9
