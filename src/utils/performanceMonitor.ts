...
export class PerformanceMonitor {
    ...
    try {
      const metrics: PerformanceMetrics = {
        ...
        memory: this.config.enableMemoryMonitoring
          ...
          : null,
        timing: this.config.enableTimingMonitoring
          ? this.getTimingMetrics()
          : null,
        network: this.config.enableNetworkMonitoring
    ...
    if (metrics.timing) {
      if (metrics.timing.loadTime > this.config.timingThreshold) {
        await this.createAlert(
          ...
          metrics.timing.loadTime,
          this.config.timingThreshold,
          metrics
        )
      }
    }
  ...
  private async createAlert(
    ...
    // 抛出性能错误
    const error = new PerformanceError(message, type, value)
