...
export class ErrorReportManager {
  ...
  async createReport(error: AppError): Promise<ErrorReportData> {
    ...
    const report: ErrorReportData = {
      ...
      performance: this.config.includePerformance ? await this.getPerformanceData() : null,
  ...
  // 发送到服务器
  private async sendToServer(report: ErrorReportData): Promise<ErrorReportResult> {
    if (!this.isOnline) {
      // 离线时加入待发送队列
  ...
  private async getRelatedLogs(error: AppError): Promise<any[]> {
    try {
      ...
      return logs.filter(log => 
        Math.abs(log.timestamp - errorTime) <= timeRange
      ).slice(0, 20);
    } catch (error) {
  ...
  private async getEnvironmentInfo(): Promise<any> {
    try {
      return {
        ...
        extension: typeof chrome !== 'undefined' && chrome.runtime ? {
  ...
  // 手动报告错误
  async reportErrorManually(error: AppError, additionalInfo: any = {}): Promise<ErrorReportResult> {
    // 增强错误上下文
    error.context = {
      ...error.context,
      additionalInfo,
...
export async function reportErrorManually(error: AppError, additionalInfo?: any): Promise<ErrorReportResult> {
