...
export class ErrorManager {
  ...
  private initializeRecoveryStrategies(): void {
    this.addRecoveryStrategy({
      ...
      canHandle: error =>
        error.type === ErrorType.NETWORK_ERROR && error.retryable,
      execute: async error => {
  ...
  captureError(
    ...
    return new AppError(type, level, message, context, { stack })
  }
  ...
  private updateStats(error: AppError): void {
    ...
    this.stats.errorsByLevel[error.level] =
      (this.stats.errorsByLevel[error.level] || 0) + 1
