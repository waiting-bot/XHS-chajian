...
export class ErrorDetailsManager {
  ...
  private updateRelatedErrors(relatedErrors: AppError[]): void {
    ...
    if (relatedElement) {
      if (relatedErrors.length > 0) {
        relatedElement.innerHTML = relatedErrors.map(error => `
  ...
  toggleSection(sectionId: string): void {
    ...
    const toggle = this.modal.querySelector(`[onclick="errorDetailsManager.toggleSection('${sectionId}')"] .section-toggle`);
