...
export class LogManager {
  ...
  private getDefaultConfig(): LogConfig {
    return {
      maxLogEntries: 1000,
      logLevels: [ErrorLevel.INFO, ErrorLevel.WARNING, ErrorLevel.ERROR, ErrorLevel.CRITICAL],
  ...
  // 控制台日志
  private logToConsole(entry: LogEntry, method: 'log' | 'info' | 'warn' | 'error' | 'debug'): void {
  ...
  private async flushPendingReports(): Promise<void> {
    ...
    try {
      const pendingLogs = this.logs.filter(log => 
        log.errorData && 
        LOG_LEVEL_CONFIG[log.level].report
      );
