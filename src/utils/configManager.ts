...
export class ConfigManager {
  ...
  private async saveConfig(config: StorageConfig): Promise<void> {
    try {
      ...
      const encryptedConfig =
        await encryptionManager.encryptSensitiveFields(config)

      // 保存到存储
      await storageManager.set(this.STORAGE_KEY, encryptedConfig, {
        encrypt: true,
      })
    ...
    try {
      const fixedConfig: StorageConfig = {
        ...
        tableDataConfig: {
          ...
          allowedTypes: config.tableDataConfig?.allowedTypes || [
            'image/jpeg',
            'image/png',
            'image/webp',
            'video/mp4',
          ...
          enableBatchProcessing:
            config.tableDataConfig?.enableBatchProcessing ?? true,
          maxConcurrentUploads:
            config.tableDataConfig?.maxConcurrentUploads || 3,
        },
        appConfig: {
      ...
      // 确保活动配置存在
      if (
        !fixedConfig.feishuConfigs.find(
          c => c.id === fixedConfig.activeConfigId
        )
      ) {
  ...
  async getActiveConfig(): Promise<FeishuConfig | null> {
    ...
    return (
      config.feishuConfigs.find(c => c.id === config.activeConfigId) || null
    )
  }
  ...
  async deleteFeishuConfig(id: string): Promise<void> {
    ...
    if (
      ...
      storageConfig.feishuConfigs.length > 0
    ) {
  ...
  // 更新表格数据配置
  async updateTableDataConfig(
    updates: Partial<TableDataConfig>
  ): Promise<TableDataConfig> {
    const storageConfig = await this.getConfig()
    const updatedConfig = {
  ...
  async validateConfig(): Promise<ValidationResult> {
    try {
      ...
      // 验证飞书配置
      const activeConfig = config.feishuConfigs.find(
      ...
      // 验证表格数据配置
      const tableDataValidation = validateTableDataConfig(
