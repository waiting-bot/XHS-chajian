...
export class SafeStorage {
  ...
  private static async wrapStorageOperation<T>(
    ...
    return new Promise<T>((resolve, reject) => {
      try {
          ...
          result
            .then((value) => {
              ...
              if (chrome.runtime && chrome.runtime.lastError) {
                const error = new Error(`[SafeStorage] Chrome storage ${operationType} error for keys [${keys.join(', ')}]: ${chrome.runtime.lastError.message}`);
            ...
            .catch((error) => {
              const wrappedError = new Error(`[SafeStorage] Storage ${operationType} operation failed for keys [${keys.join(', ')}]: ${error.message}`);
        ...
        const wrappedError = new Error(`[SafeStorage] Storage ${operationType} operation failed for keys [${keys.join(', ')}]: ${error instanceof Error ? error.message : String(error)}`);
        console.error(wrappedError.message);
        reject(wrappedError);
      }
    });
  ...
  static async get(keys: string | string[]): Promise<Record<string, any>> {
    ...
    const validKeys = keyArray.filter(key => 
