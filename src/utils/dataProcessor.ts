...
export class DataProcessor {
  ...
  public async testFieldMapping(): Promise<{
    success: boolean
    errors: string[]
  }> {
    try {
    ...
    try {
      ...
      return {
        ...
        lastProcessed: appConfig.statistics.lastProcessedAt
          ? new Date(appConfig.statistics.lastProcessedAt)
          : undefined,
      }
    } catch {
      return {
              ...
              (progress, message) => {
                const totalProgress =
                  ((this.queue.length + index) /
                    (this.queue.length + batch.length)) *
                  100
