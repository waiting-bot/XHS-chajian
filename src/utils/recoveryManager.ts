...
export class RecoveryManager {
  ...
  private initializeDefaultStrategies(): void {
    ...
    this.addStrategy({
      ...
      canHandle: error => {
        return [ErrorType.DOM_ERROR, ErrorType.EXTENSION_ERROR].includes(
          error.type
        )
      },
      execute: async error => {
      ...
      execute: async error => {
        ...
        const delay = Math.min(
          this.config.retryDelay *
            Math.pow(2, error.context?.additionalData?.attempt || 1),
          10000
        )
    ...
    this.addStrategy({
      ...
      canHandle: error => {
        return [ErrorType.NETWORK_ERROR, ErrorType.AUTH_ERROR].includes(
          error.type
        )
      },
      execute: async error => {
  ...
  private async attemptRecovery(
    ...
    for (const strategyId of this.config.recoveryStrategies) {
      ...
      try {
        ...
        console.log(
          `恢复策略 ${strategy.name} ${success ? '成功' : '失败'}，耗时 ${duration}ms`
        )
...
export async function retryWithRecovery<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // 尝试恢复
      const strategies = recoveryManager
        .getAllStrategies()
        .filter(s => s.canHandle(appError))
        .map(s => s.id)

      if (strategies.length > 0) {
        const results = await recoveryManager.manualRecovery(
          appError,
          strategies
        )
