...
function renderConfigStatus(status: ConfigStatus) {
  ...
  statusIcon.className = status.isConfigured
    ? 'status-icon success'
    : 'status-icon error'
  ...
  timeText.textContent = status.lastUpdated
    ? `更新于: ${status.lastUpdated}`
    : ''
...
class PopupManager {
  ...
  private initializeEventListeners(): void {
    ...
    chrome.storage.onChanged.addListener((changes, namespace) => {
      if (
        namespace === 'sync' &&
        (changes.feishuConfigs || changes.currentConfigId)
      ) {
        this.loadConfigs()
      }
    })
  ...
  private async collectAndWriteToFeishu(): Promise<void> {
    ...
    try {
      ...
      const contentResponse = await chrome.tabs.sendMessage(tab.id, {
        action: 'collectNoteData',
      })

      if (!contentResponse || !contentResponse.success) {
        throw new Error(
          contentResponse?.error || '页面数据采集失败，请刷新页面重试'
        )
      }
  ...
  private updateConfigSelector(): void {
    ...
    configSelect.innerHTML = this.configs
      .map(
        config =>
  ...
  private getCurrentConfig(): any {
    return (
      this.configs.find(config => config.id === this.currentConfigId) || null
    )
  }
  ...
  private async saveConfig(): Promise<void> {
    ...
    const appId = (
      document.getElementById('appId') as HTMLInputElement
    ).value.trim()
    const tableId = (
      document.getElementById('tableId') as HTMLInputElement
    ).value.trim()
    const accessToken = (
  ...
  private validateNoteInput(): boolean {
    ...
    // 移除之前的验证消息
    const existingMessage = noteInput.parentElement?.querySelector(
      '.validation-message'
    )
    if (existingMessage) {
  ...
  private updateTagDisplay(): void {
    ...
    if (selectedTagsContainer) {
      selectedTagsContainer.innerHTML = Array.from(this.selectedTags)
        .map(
          tag =>
  ...
  private clearNote(): void {
    ...
    // 重置分类选择
    const generalCategory = document.querySelector(
      'input[name="noteCategory"][value="general"]'
    ) as HTMLInputElement
    if (generalCategory) {
  ...
  private updateHistoryDisplay(): void {
    ...
    if (this.noteHistory.length === 0) {
      historyList.innerHTML =
        '<div class="history-placeholder">暂无历史记录</div>'
      return
    }

    historyList.innerHTML = this.noteHistory
      .map(
        note => `
...
function toggleCard(contentId: string): void {
  ...
  const button =
    content?.previousElementSibling?.querySelector('.expand-button')
