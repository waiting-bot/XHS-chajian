...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const config = {
        ...
        feishuBaseUrl: formData.get('baseUrl') as string || 'https://open.feishu.cn',
  ...
  async function loadConfig() {
    try {
      ...
      // 检查连接状态
      if (result.feishuAppId && result.feishuAppSecret && result.feishuAppToken) {
  ...
  async function testConnection() {
    ...
    try {
      ...
      if (!config.feishuAppId || !config.feishuAppSecret || !config.feishuAppToken) {
  ...
  // 解析飞书表格URL
  function parseSpreadsheetUrl(url: string): { appToken: string; tableId: string } {
    try {
      ...
      if (!appToken) {
        const sheetsPathMatch = urlObj.pathname.match(/\/sheets\/([a-zA-Z0-9]+)/);
        if (sheetsPathMatch) {
      ...
      throw new Error('URL格式不正确，无法解析App Token和Table ID。请确保使用正确的飞书多维表格链接。');
    }
  }
  ...
  // 更新连接状态显示
  function updateConnectionStatus(status: 'connected' | 'disconnected' | 'testing', message: string) {
    const indicator = connectionStatus.querySelector('.status-indicator') as HTMLElement;
    
    // 移除所有状态类
    indicator.classList.remove('status-connected', 'status-disconnected', 'status-testing');
